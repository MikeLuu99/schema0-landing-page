...
export default function PricingSection() {
  ...
  const [buildType, setBuildType] = useState<"build" | "diy">("build")

  // Calculate prices based on billing period
  const getPrice = (monthly: number) => {
    if (isYearly) {
      return Math.floor(monthly * 12 * 0.75) // 25% discount for yearly
    }
    return monthly
  }
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-4 mb-12">
          ...
          <h1 className="text-gray-900 text-4xl md:text-5xl font-normal">Plans and Pricing</h1>
          <p className="text-gray-600 text-lg">Flexible database and AI solutions for projects of all sizes.</p>
        </div>

        {/* Toggle Buttons */}
        <div className="flex flex-col gap-8 mb-12">
          <div className="inline-flex p-1 bg-white rounded-lg self-start shadow-sm">
            <Button
              variant="ghost"
              className={`px-4 py-2 rounded-md ${
                buildType === "build" ? "bg-gray-900 text-white" : "text-gray-600 hover:text-gray-900"
              }`}
            ...
            <Button
              variant="ghost"
              className={`px-4 py-2 rounded-md ${
                buildType === "diy" ? "bg-gray-900 text-white" : "text-gray-600 hover:text-gray-900"
              }`}
              onClick={() => setBuildType("diy")}
            >
              Do It Yourself
            </Button>
          </div>
          ...
          <div className="flex items-center gap-3 self-end">
            ...
            {isYearly && <span className="text-sm text-green-600">(Save 25%)</span>}
            <Switch
              checked={!isYearly}
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="border border-gray-300 rounded-lg p-6 space-y-6 bg-white shadow-sm">
            <div className="space-y-2">
              ...
              <p className="text-gray-600 text-sm">Perfect for side projects and learning</p>
            </div>
          ...
          <div className="border border-gray-300 rounded-lg p-6 space-y-6 bg-white shadow-sm">
            <div className="space-y-2">
              ...
              <p className="text-gray-600 text-sm">For growing teams and applications</p>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-gray-900 text-3xl font-normal">${getPrice(29)}</span>
              <span className="text-gray-600">/{isYearly ? "year" : "month"}</span>
            </div>
            <Button className="w-full bg-gray-900 text-white hover:bg-gray-800">Start free trial</Button>
            <ul className="space-y-4">
              <li className="flex gap-2 text-gray-600">
                <Check className="w-5 h-5 text-gray-900" />
